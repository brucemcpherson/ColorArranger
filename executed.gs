'use strict';
/**
 * This detects the most colorful column if none are explicitly specified, falling back to the last column
 * @param {number} target the target column or -ve if not explicit
 * @return {number} the column  to use
 */
function detectColorfulColumn(target) {
  
  if(target <0) {
    // we have to read all the data, and go for the one with the maximum variety of colors more than 2
    var best = target, bestCount = 2;
    
    // turn the data columnwise
    transpose(SpreadsheetApp.getActiveSheet().getDataRange().getBackgrounds()).forEach (function (d,i,a) {
       var k = colorCount(d);
  ...
  // count how many distinct colors there are
  function colorCount(column) {
    var c = 0;
    
    column.forEach(function (d,i,a) {
      if (a.indexOf(d) === i) c++;
    });
    return c;
  }
...
function getSheetData (target) {
  ...
  var best = detectColorfulColumn (target);
  
  // nothing to choose between them, use the current column
  var currentColumn  = best >=0 ? best+1  : sh.getActiveCell().getColumn();
